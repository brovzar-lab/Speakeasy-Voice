...
- Providers: Apple (local, free, uses downloadable premium voices) is default. ElevenLabs and OpenAI are cloud fallbacks — API keys stored via `APIKeyManager` under provider ids `"elevenlabs"` and `"openai"`.
...
- Read Aloud speed tips: Apple (local) is instant; ElevenLabs Flash v2.5 + streaming is the fast cloud path; Turbo is slower at the same price. One-time migration flag `readAloud.migratedElevenLabsFlash_v1` bumps stored Turbo users to Flash.

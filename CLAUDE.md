...
- Read Aloud speed tips: Apple (local) is instant; ElevenLabs Flash v2.5 + streaming is the fast cloud path; Gemini **3.1 Flash** streams via `streamGenerateContent?alt=sse` (default; one-time migration `readAloud.migratedGemini31Flash_v1` bumps prior 2.5 defaults). Gemini 2.5 Flash is cheapest but waits for the full clip. Turbo is slower at the same ElevenLabs price. One-time migration flag `readAloud.migratedElevenLabsFlash_v1` bumps stored Turbo users to Flash.

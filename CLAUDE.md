...
## Stack
- Current user-visible version: **1.7** (`MARKETING_VERSION = 1.7`, build 203). The version appears below Settings in the sidebar.
...
- Read Aloud PCM playback fix (2026-07-16): the Float32 player-node → `AVAudioUnitVarispeed` → mixer graph still failed to initialize with `kAudioUnitErr_FormatNotSupported (-10868)` on the active macOS 26 output route, causing Local HD to silently stop. `CloudPCMPlaybackFormat` converts network PCM to deinterleaved native Float32 and applies speed with software resampling. Keep the graph as direct player-node → mixer; do not restore a streaming effect unit or connect Int16 wire data to the graph.

...
## Stack
- Current user-visible version: **1.7** (`MARKETING_VERSION = 1.7`, build 205). The version appears below Settings in the sidebar.
...
- Read Aloud fallback UI crash (2026-07-16): provider recovery used an unisolated callback, so Local HD failure could create its fallback `NSPanel` on a cooperative background thread and terminate with `NSWindow should only be instantiated on the main thread!`. `ReadAloudPlaybackRecovery` now requires a `@MainActor` fallback callback and reports the underlying provider error. Keep all AppKit fallback UI on the main actor.
- Read Aloud PCM playback fix (2026-07-16): the Float32 player-node → `AVAudioUnitVarispeed` → mixer graph still failed to initialize with `kAudioUnitErr_FormatNotSupported (-10868)` on the active macOS 26 output route, causing Local HD to silently stop. `CloudPCMPlaybackFormat` converts network PCM to deinterleaved native Float32 and applies speed with pitch-preserving WSOLA time stretching. Keep the graph as direct player-node → mixer; do not restore a streaming effect unit, simple resampling, or Int16 wire data in the graph.

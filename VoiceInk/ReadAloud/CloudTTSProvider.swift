...
enum CloudPCMPlaybackFormat {
    ...
    @discardableResult
    static func copyPCM16LittleEndian(_ data: Data, into buffer: AVAudioPCMBuffer) -> Bool {
...
final class CloudTTSPlayer: NSObject, AVAudioPlayerDelegate {
    ...
    /// Stream native Float32 mono samples generated by an on-device model.
    /// The engine stays alive across every planned text segment, so locally
    /// generated long reads do not pause at paragraph boundaries.
    func playStreamingFloat32(
    ...
    private func scheduleStreamingPCMBuffer(_ buffer: AVAudioPCMBuffer) {
        guard let node = streamingNode else { return }
